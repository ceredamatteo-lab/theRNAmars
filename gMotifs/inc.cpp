...
void options::print(ostream & out) {
 ...
 cout << "enrichment window:\t" << enrichment_window << endl;
...
options::options(int argc,char *argv[]) {
 ...
 if(in.is_open()){
 	...
 	while(getline(in,line)){
      ...
      enrichment_window=atoi(param.c_str());
    }
   	i++;
  }
...
int count_per_regions(options &opt,const char *sp_fname,const char *bed_fname, const char *output_fname){
	try{
		...
		string line,f;
		vector<bln> bedlines, bedres;
		unsigned int CEone = 0, CEminone = 0, CEzero = 0;
		
		fout << "myRID\trowID\ttype\thits_region1\thits_region2\thits_region3\n"; 

		if(in.is_open() && fbed.is_open()){
      // Read BED file
			while(getline(fbed,line)) bedlines.push_back(bln(line)); 
			fbed.close();
      // read splicing change lines and evaluate bed positions
			while(getline(in,f)){  
				gString spline=f; 
				vector<gString> pars;
				spline.split(';',pars);
        ...
        unsigned int r1_start            = in_start - 5 - opt.enrichment_window ,
                     r1_stop             = in_start - 5;
				...
				unsigned int r3_start            = in_stop  + 10,
                     r3_stop             = in_stop  + 10 + opt.enrichment_window;
				...
				bool hits_region1 = false, hits_region2 = false, hits_region3 = false;
				...
				for(unsigned int g=0; g<bedlines.size();g++){
					if(strcmp(chr.c_str(),bedlines[g].chr)==0 &&
					   (forw==bedlines[g].forw)){ 
							...
							if (!forw){
								r1_start            = in_stop + 5, 
								r1_stop             = in_stop + 5 + opt.enrichment_window , // piu alta
								r3_start            = in_start - 10 - opt.enrichment_window, // piu bassa
								r3_stop             = in_start - 10; 
							}
