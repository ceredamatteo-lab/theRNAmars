...
getTables = function(protein,place,tets, inExon    = 50, inIntron  = 200){
  ...
  if (any(!is.na(overlap))) {  
    ...
    rown = c((regions*1-inExon):(regions*1+inIntron),
             ...
             (regions*3-inExon):(regions*3+inIntron),
             (regions*4-inIntron):(regions*4+inExon))
    tab = matrix(0, nrow=length(rown), ncol=length(sel.tets) )
    rownames(tab) = rown
    ...
    for (i in 1:length(filelist)){
      if(!is.na(i)){
        ...
        if(!is.null(df) && nrow(df)>1){
          ...
          tet = sel.tets[i];
          #################################################################################
          chosen = subset(df, cat==1); rownames(chosen) = as.character(chosen$pos);	res[["enh"]][match(chosen$pos, rown)[which(!is.na(match(chosen$pos, rown)))],tet]   = chosen[which(chosen$pos %in% rown),"no"]
...
rnaScoreMap=function(data,rcol, ylabels=c("0.5","1"),main="",exon=50,intron=200,gap=10,regions=2000,ylim.redox=F){
  ...
  p1=xyplot(s~plot|g,data=pf_real,
            ...
            panel = function(x,y,hrect,rcol,intensity,groups,subscripts,...){
              panel.xblocks(x=offset$start[1]:offset$end[4],
                            c(rep("0",exon),rep(NA,intron+1),rep(NA,gap-1),rep(NA,intron+1),rep("3",exon),
                              rep(NA,gap-1),rep("4",exon),rep(NA,intron+1),rep(NA,gap-1),rep(NA,intron+1),rep("7",exon)),
                            col = "gray96",border = FALSE,lwd=0.5)
              panel.xyplot(x,y,...)
              panel.xblocks(x=offset$start[1]:offset$end[4],
                            c(rep(NA,exon+intron+1),rep("0",gap-1),rep(NA,intron+exon+1),rep("0",gap-1),rep(NA,exon+intron+1),rep("0",gap-1),rep(NA,intron+exon+1)),
